...
@Component({
  moduleId: module.id,
  selector: 'ng-tree-out',
  templateUrl: 'tree.out.html'
  // directives: [NgTableSortingDirective, NgClass, CORE_DIRECTIVES, PAGINATION_DIRECTIVES, PaginationComponent]
})
export class TreeOutComponent implements OnInit, OnChanges {
  ...
  public addHoverDom(treeId: any, treeNode: any) {
    ...
    //停车场只允许修改，不允许添加与删除
    if (treeNode.isBelongPark && treeNode.isBelongPark === true) {
      //console.log('!!!!!!与停车场相关');
    } else {
      //添加
  ...
  public treeItemClick(event: any, treeId: any, treeNode: any) {
    if (!this.isLock) {
      ...
      this.itemClicked.emit(treeNode);
    }
  }
